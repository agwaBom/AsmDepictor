...
class IDA(object):
    ...
    Useful IDA-provided APIs
        IDC
            ...
            idc.GetManyBytes(ea, num): Returns num of bytes at the given address
            ...
            idc.ItemEnd(ea): Returns the end of the instruction (or item)
            idc.GetFunctionName(ea): Returns the name of the function that ea belongs to
            idc.GetDisasm(ea): Returns the disassembly at ea
            idc.GetType(fn_ea): Returns the type of the function at fn_ea
            idc.SegName(ea): Returns the name of the section that ea belongs to
        IDAUtils
    ...
    def update_refs(self, ea):
        ...
        for xref in idautils.XrefsTo(ea, flags=idaapi.XREF_ALL):
            ...
            if idc.get_segm_name(xref_addr) == ".text":
                ...
                try:
                    caller_start = idaapi.get_func(xref_addr).startEA
    ...
    def run(self):
        ...
        for f_ea in idautils.Functions():
            if idc.get_segm_name(f_ea) == ".text":
                ida_ft = ida_funcs.get_func(f_ea)
                ...
                F.no_return = idc.get_func_attr(f_ea, idc.FUNCATTR_START) & ida_funcs.FUNC_NORET
                ...
                for bb in bbs:
                    ...
                    while insn_addr < bb_end:
                        ...
                        I.is_call = ida_idp.is_call_insn(insn_start)
                        if I.is_call:
                            try:
...
if __name__ == "__main__":
    ...
    ida_auto.auto_wait()  # Wait for IDA to complete its auto analysis
