...
class Normalization():
    ...
    def disassemble_and_normalize_instructions(self, normalization_level=3):
        ...
        def __normalize_immediate2(I, opr):
            opcode = I.opcode
        ...
        def __normalize_ptr2(operand, ref_str_access=False):
            """
                                ...
                                try:
                                    ...
                                    if normalization_level > 0: # immediate value
                                        ...
                                        if opr.access == CS_AC_INVALID:
                                            operands[idx] = __normalize_immediate2(I, operands[idx])
    ...
    def pickle_dump(self, BS):
        ...
        while fn:
            ...
            BSFS.ref_tos_by_call = [self.dumped_funcs[x].name for x in fn.call_refs_to] \
                if len(fn.call_refs_to) > 0 else []
            BSFS.ref_tos_by_jump = [self.dumped_funcs[x].name for x in fn.jump_refs_to] \
                if len(fn.jump_refs_to) > 0 else []
            BSFS.ref_tos_by_data = [self.dumped_funcs[x].name for x in fn.data_refs_to] \
                if len(fn.data_refs_to) > 0 else []
            BSFS.ref_froms = [self.dumped_funcs[x].name for x in fn.refs_from] \
...
if __name__ == '__main__':
    ...
    # dumped file input (dmp.gz)
    target = "./sample_binary/gcc-5__Ou__direvent__direvent"

    file_name = os.path.basename(target)
    ida_dmp_path = target + ".dmp.gz"
    pkl_dmp_path = file_name + ".pkl"
    txt_dmp_path = file_name + ".info.txt"

    #_, arch, compiler, optlevel = file_name.split("-")
